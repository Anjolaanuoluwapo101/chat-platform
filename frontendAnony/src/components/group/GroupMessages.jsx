...
const GroupMessages = () => {
    ...
    const [showAdminPanel, setShowAdminPanel] = useState(false); // false or string ('members', 'banned', 'settings')

    const scrollToBottom = useCallback(() => {
        messagesEndRef.current?.scrollIntoView({ behavior: "smooth" });
    }, []);

    const loadMessages = useCallback(async (referenceId = null, direction = 'before') => {
        try {
            if (referenceId) setLoadingMore(true);
            
            // Backend will automatically use last_read_id if no referenceId provided
            const res = await groupService.getGroupMessages(groupId, referenceId, direction);
    ...
    const handleSend = async (message, files, replyToMessageId = null) => {
        try {
            await groupService.sendGroupMessage(groupId, message, files, replyToMessageId);
        } catch (err) {
