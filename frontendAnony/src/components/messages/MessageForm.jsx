...
const MessageForm = ({ onMessageSent = () => {}, replyToMessage = null, onCancelReply = () => {} }) => {
      ...
      {replyToMessage && (
        <div className="px-4 py-2 bg-blue-50 border-b border-blue-200 flex items-start justify-between">
          <div className="flex-1 min-w-0">
            ...
            {replyToMessage.mediaUrls && replyToMessage.mediaUrls.length > 0 && (
              <div className="mt-1 flex flex-wrap gap-1">
                {replyToMessage.mediaUrls.slice(0, 3).map((url, idx) => (
                  <div key={idx} className="w-8 h-8 bg-gray-200 rounded overflow-hidden">
                    {renderMedia(url, idx)}
                  </div>
                ))}
                {replyToMessage.mediaUrls.length > 3 && (
