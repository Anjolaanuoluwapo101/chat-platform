...
class PostgresDatabase extends AbstractSQLDatabase
{
    ...
    protected function getDSN($config)
    {
        if (!empty($config['url'])) {
            ...
            // Allow user and password to be parsed from the URL
            if (isset($parsed['user'])) {
                $dsn .= ";user={$parsed['user']}";
            }
            if (isset($parsed['pass'])) {
                $dsn .= ";password={$parsed['pass']}";
            }
    ...
    protected function createTables(): void
    {
        ...
        // Groups table (quoted to avoid reserved keyword conflicts)
        $this->pdo->exec("
    ...
    public function saveUser($user)
    {
        try {
            ...
            $stmt = $this->pdo->prepare("INSERT INTO users (username, password_hash, email, verification_code, is_verified) VALUES (?, ?, ?, ?, ?) RETURNING id");
            $stmt->execute([
                $user['username'],
                $user['password_hash'],
                $user['email'] ?? null,
        ...
        try {
            $stmt = $this->pdo->prepare("
                SELECT
                    ...
                    m.content,
                    m.created_at,
                    STRING_AGG(DISTINCT p.file_path, ',') as photos,
                    STRING_AGG(DISTINCT v.file_path, ',') as videos,
                    STRING_AGG(DISTINCT a.file_path, ',') as audios
                FROM messages m
    ...
    public function getMessageById($messageId, $groupId = null)
    {
        try {
            $stmt = $this->pdo->prepare("
                ...
                LEFT JOIN audio a ON m.id = a.message_id
                WHERE m.id = ?
                GROUP BY m.id, m.content, m.created_at, m.group_id, u.username, g.is_anonymous
            ");
    ...
    {
        try {
            $stmt = $this->pdo->prepare("INSERT INTO \"groups\" (name, is_anonymous, created_at) VALUES (?, ?, CURRENT_TIMESTAMP) RETURNING id");
