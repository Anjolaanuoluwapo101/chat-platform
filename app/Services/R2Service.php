<?php

namespace App\Services;

use App\Config\Config;
use Aws\S3\S3Client;
use Aws\Exception\AwsException;
use Aws\Credentials\Credentials;
use App\Log\Logger;

class R2Service
{
    private $s3;
    private $bucketName;
    private $publicDomain;
    private $logger;

    public function __construct()
    {
        $this->logger = new Logger();

        $accountId = Config::get('r2')['account_id']; 
        $accessKey = Config::get('r2')['access_key_id'];
        $secretKey = Config::get('r2')['secret_access_key'];

        //check if any of the required values are missing
        if (empty($accountId) || empty($accessKey) || empty($secretKey)) {
            throw new \Exception("Missing R2 credentials. Please check your configuration.");
        }
        
        $this->bucketName = Config::get('r2')['bucket_name']; // The name you gave your bucket
        
        // 2. SET PUBLIC DOMAIN 
        $this->publicDomain = Config::get('r2')['public_bucket_url']; 

        $credentials = new Credentials($accessKey, $secretKey);

        // 3. INITIALIZE CLIENT
        $this->s3 = new S3Client([
            'region' => 'auto', // R2 ignores region, but SDK requires it
            'version' => 'latest',
            'endpoint' => "https://$accountId.r2.cloudflarestorage.com",
            'credentials' => $credentials
        ]);
    }

    /**
     * Upload a file to R2
     */
    public function uploadFile($tempFilePath, $fileName, $mimeType)
    {
        try {
            // Upload the file
            $this->s3->putObject([
                'Bucket' => $this->bucketName,
                'Key'    => $fileName,      
                'SourceFile' => $tempFilePath, 
                'ContentType' => $mimeType,
                // 'ACL' => 'public-read' // R2 doesn't always use ACLs same as AWS, relies on bucket settings
            ]);

            // Construct the public URL manually
            // R2 URLs are: PublicDomain + / + FileName
            return $this->publicDomain . '/' . $fileName;

        } catch (AwsException $e) {
            // Log error locally
            $this->logger->log("R2 Upload Error: " . $e->getMessage());
            throw new \Exception("Failed to upload file to storage.");
        }
    }

    /**
     * Delete a file from R2
     */
    public function deleteFile($fileName)
    {
        try {
            $this->s3->deleteObject([
                'Bucket' => $this->bucketName,
                'Key'    => $fileName
            ]);
            return true;
        } catch (AwsException $e) {
            error_log("R2 Delete Error: " . $e->getMessage());
            return false;
        }
    }
}