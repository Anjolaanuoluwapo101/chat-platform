...
export const ChatHeader = ({ 
  ...
  <header className="px-4 py-3 bg-linear-to-b from-gray-100 to-gray-200 border-b border-gray-300 flex justify-between items-center">
 ...
 */
export const JoinGroupView = ({ onJoin }: { onJoin: () => void }) => (
...
export const MessageFormContainer = ({ onSubmit, children }: { onSubmit: (e: React.FormEvent) => void; children: ReactNode }) => (
  <form onSubmit={onSubmit} className="flex items-end p-3 bg-linear-to-b from-gray-100 to-gray-200 border-t border-gray-300">
    {children}
  </form>
);
...
export const SendButton = ({ disabled = false, loading = false }: { disabled?: boolean; loading?: boolean }) => (
  <button 
    ...
    className="ml-2 px-4 py-2 bg-linear-to-b from-green-400 to-green-500 text-white font-medium rounded-2xl shadow-sm hover:from-green-500 hover:to-green-600 disabled:opacity-50 transition-all"
  >
    {loading ? (
